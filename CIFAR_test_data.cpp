...
{
    /// Sanity check of the insert_L1_IN_convolution_cube
    if(insert_L1_IN_convolution_cube.type() != 21)
    {
        printf("ERROR! Mat type is wrong insert_L1_IN_convolution_cube.type() = %d is not = CV_32FC3 = 21\n", insert_L1_IN_convolution_cube.type());
        exit(0);
    }
    if(insert_L1_IN_convolution_cube.cols != CIFAR_width)
    {
        printf("ERROR! insert_L1_IN_convolution_cube.cols = %d != CIFAR_width\n", insert_L1_IN_convolution_cube.cols);
        exit(0);
    }

    if(insert_L1_IN_convolution_cube.rows != CIFAR_height)
    {
        printf("ERROR! insert_L1_IN_convolution_cube.rows = %d != CIFAR_height\n", insert_L1_IN_convolution_cube.rows);
        exit(0);
    }
...
///    insert_L1_IN_convolution_cube = cv::Scalar(0,0,0);
///    insert_L1_IN_convolution_cube.at<float>(5, 0) = 1.0;/// 0 column = Blue
///    insert_L1_IN_convolution_cube.at<float>(6, 1) = 1.0;/// 1 column = Green
///    insert_L1_IN_convolution_cube.at<float>(7, 2) = 1.0;/// 2 column = Red
    ///==============================================
    CIFAR_nr = (int) (rand() % CIFAR_nr_of_img_p_batch);
    printf("Load CIFAR_nr = %d to insert_L1_IN_convolution_cube\n", CIFAR_nr);
    ...
    for (int i=0; i<(CIFAR_height*CIFAR_width); i++)
    {
        for(int BGR=0; BGR<3; BGR++)
        {
            ...
            ///   example_mat.at<float>(ROW, COLUMN) = 1.0f;
            insert_L1_IN_convolution_cube.at<float>(i/(CIFAR_height*CIFAR_width), (i%(CIFAR_height*CIFAR_width)*3) + BGR) = pixel_data;
        }
    }
